...
import com.jiaye.cashloan.http.gongxinbao.GongXinBaoOperators;
...
import com.jiaye.cashloan.http.gongxinbao.GongXinBaoToken;
...
public class LoanAuthPhoneActivity extends AppCompatActivity {
    ...
    private void request() {
        Disposable disposable = Flowable.just("")
                ...
                .flatMap(new Function<GongXinBaoTokenRequest, Publisher<GongXinBaoResponse<GongXinBaoToken>>>() {
                    @Override
                    public Publisher<GongXinBaoResponse<GongXinBaoToken>> apply(GongXinBaoTokenRequest request) throws Exception {
                        return GongXinBaoClient.INSTANCE.getService().auth(request);
                    }
                })
                .map(new GongXinBaoResponseFunction<GongXinBaoToken>())
                .map(new Function<GongXinBaoToken, GongXinBaoToken>() {
                    @Override
                    public GongXinBaoToken apply(GongXinBaoToken token) throws Exception {
                        mToken = token.getToken();
                        ContentValues values = new ContentValues();
                        values.put("gxb_token", token.getToken());
                        LoanApplication.getInstance().getSQLiteDatabase().update("user", values, null, null);
                        return token;
                    }
                })
                .flatMap(new Function<GongXinBaoToken, Publisher<GongXinBaoResponse<GongXinBaoOperators>>>() {
                    @Override
                    public Publisher<GongXinBaoResponse<GongXinBaoOperators>> apply(GongXinBaoToken token) throws Exception {
                        return GongXinBaoClient.INSTANCE.getService().operatorsAuth(token.getToken(), mPhone);
                    }
                })
                .map(new GongXinBaoResponseFunction<GongXinBaoOperators>())
                ...
                .map(new Function<GongXinBaoOperators, Boolean>() {
                    @Override
                    public Boolean apply(GongXinBaoOperators operators) throws Exception {
                        ...
                        GongXinBaoOperators.LoginForms.Fields[] fields = operators.getLoginForms()[0].getFields();
                        boolean isNeedRequestImgVerificationCode = false;
                        for (GongXinBaoOperators.LoginForms.Fields field : fields) {
                ...
                .flatMap(new Function<Boolean, Publisher<GongXinBaoResponse<GongXinBaoAuth>>>() {
                    @Override
                    public Publisher<GongXinBaoResponse<GongXinBaoAuth>> apply(Boolean aBoolean) throws Exception {
                        return GongXinBaoClient.INSTANCE.getService().refreshVerifyCode(mToken);
                    }
                })
                .map(new GongXinBaoResponseFunction<GongXinBaoAuth>())
                .map(new Function<GongXinBaoAuth, Bitmap>() {
                    @Override
                    public Bitmap apply(GongXinBaoAuth response) throws Exception {
    ...
    private void requestSMSVerification() {
        Disposable disposable = GongXinBaoClient.INSTANCE.getService().refreshSmsCode(mToken)
                .compose(new ViewTransformer<GongXinBaoResponse<GongXinBaoAuth>>())
                .subscribe(new Consumer<GongXinBaoResponse<GongXinBaoAuth>>() {
                    @Override
                    public void accept(GongXinBaoResponse<GongXinBaoAuth> response) throws Exception {
    ...
    private void requestIMGVerification() {
        Disposable disposable = GongXinBaoClient.INSTANCE.getService().refreshVerifyCode(mToken)
                .subscribeOn(Schedulers.io())
                .doOnSubscribe(new Consumer<Subscription>() {
                    @Override
                    public void accept(Subscription subscription) throws Exception {
    ...
    private void commit() {
        if (isSecond) {
            ...
            Disposable disposable = GongXinBaoClient.INSTANCE.getService().login(mToken, request)
                    .map(new GongXinBaoResponseFunction<GongXinBaoAuth>())
                    .subscribeOn(Schedulers.io())
                    .doOnSubscribe(new Consumer<Subscription>() {
                        @Override
                    ...
                    .subscribeOn(AndroidSchedulers.mainThread())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new Consumer<GongXinBaoAuth>() {
                        @Override
                        public void accept(GongXinBaoAuth response) throws Exception {
                            polling();
                        }
                    }, new Consumer<Throwable>() {
                        @Override
    ...
    private void polling() {
        mIsPollingEnd = false;
        Disposable disposable = GongXinBaoClient.INSTANCE.getService().getLoginStatus(mToken)
                .map(new GongXinBaoResponseFunction<GongXinBaoAuth>())
                .map(new Function<GongXinBaoAuth, GongXinBaoAuth>() {
                    @Override
                    public GongXinBaoAuth apply(GongXinBaoAuth response) throws Exception {
    ...
    private void commitSecond() {
        ...
        Disposable disposable = GongXinBaoClient.INSTANCE.getService().second(mToken, request)
                .map(new GongXinBaoResponseFunction<GongXinBaoAuth>())
                .subscribeOn(Schedulers.io())
                .doOnSubscribe(new Consumer<Subscription>() {
                    @Override
